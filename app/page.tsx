...
export default function App() {
  ...
  const [translations, setTranslations] = useState<TranslationBlock[] | null>(
    null,
  );
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [defaultFilename, setDefaultFilename] =
    useState<string>("翻訳済み画像");
  const [isGeneratingFilename, setIsGeneratingFilename] =
  ...
  const isApiReady =
    Boolean(process.env.NEXT_PUBLIC_GEMINI_API_KEY) || Boolean(geminiKey);
  ...
  const handleTranslate = async () => {
    ...
    if (!isApiReady) {
      setError(
        "APIキーが設定されていません。右上の設定(⚙)からGemini APIキーを設定してください。",
      );
    ...
    try {
      ...
      const apiTranslations = await translateImageText(
        base64Image,
      ...
      <div className="bg-background-light dark:bg-background-dark min-h-screen flex text-slate-800 dark:text-slate-100 items-center justify-center p-4 transition-colors duration-300">
        <div className="fixed top-4 right-4 z-50">
          <button
            ...
            className="p-2 rounded-full bg-white dark:bg-card-dark shadow-lg border border-border-light dark:border-border-dark text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
          >
            <span className="material-icons-outlined">
              {isDarkMode ? "light_mode" : "brightness_6"}
            </span>
          </button>
        </div>
        <main className="w-full max-w-5xl bg-card-light dark:bg-card-dark rounded-2xl shadow-xl dark:shadow-2xl border border-border-light dark:border-border-dark overflow-hidden transition-colors duration-300 p-8 md:p-12 relative animate-fade-in-up">
          ...
          <header className="text-center mb-12 relative z-10">
            <div className="absolute top-0 right-0">
              <button
                ...
                aria-label="API設定を開く"
                className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 transition-colors relative"
              >
                {!isApiReady && (
            ...
            <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed mt-2">
              日本語のテキストが含まれる画像をアップロードすると、AIがレイアウト
          ...
          {error && (
            <div
              role="alert"
              aria-live="assertive"
              className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-600 dark:text-red-400 text-sm text-center"
            >
              {error}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            <div className="bg-gray-50 dark:bg-[#151e2e] rounded-xl border border-border-light dark:border-border-dark p-6 flex flex-col h-full">
              <div className="flex items-center mb-6">
                <span className="flex items-center justify-center w-6 h-6 rounded-full bg-primary text-white text-xs font-bold mr-3">
                  1
                </span>
            ...
            <div className="bg-gray-50 dark:bg-[#151e2e] rounded-xl border border-border-light dark:border-border-dark p-6 flex flex-col h-full">
              <div className="flex items-center mb-6">
                <span className="flex items-center justify-center w-6 h-6 rounded-full bg-purple-600 text-white text-xs font-bold mr-3">
                  2
                </span>
                <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100">
                  翻訳を実行
              ...
              <div className="flex-grow flex flex-col justify-center items-center text-center px-4">
                ...
                >
                  <span className="material-icons-outlined group-hover:animate-pulse">
                    translate
                  </span>
                  画像を翻訳
                </button>
        ...
        <div className="fixed bottom-4 left-4 z-50">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full px-4 py-2 shadow-lg flex items-center gap-2 transition-colors">
            <span
              className={`w-2 h-2 rounded-full ${isApiReady ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
            ></span>
            <span className="text-xs font-medium text-gray-600 dark:text-gray-300">
              {isApiReady ? "AI Engine Ready" : "API Key Required"}
            </span>
          </div>
        </div>
        ...
        <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-xl dark:shadow-2xl dark:shadow-indigo-900/20 border border-border-light dark:border-border-dark overflow-hidden transition-colors">
          <div className="relative pt-12 pb-8 px-6 text-center border-b border-border-light dark:border-border-dark bg-slate-50/50 dark:bg-slate-800/50">
            <button
              onClick={() => setIsSettingsOpen(true)}
              aria-label="API設定を開く"
              className="absolute top-6 right-6 p-2 rounded-full text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors relative"
            >
              {!isApiReady && (
                <span className="absolute top-1 right-1 flex h-2.5 w-2.5">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
          ...
          <div className="p-6 md:p-10 space-y-12">
            {error && (
              <div
                role="alert"
                aria-live="assertive"
                className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-600 dark:text-red-400 text-sm text-center"
              >
                {error}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 opacity-75 hover:opacity-100 transition-opacity duration-300">
              ...
              <div className="border border-border-light dark:border-border-dark rounded-xl p-5 bg-white dark:bg-slate-800/50 shadow-sm flex flex-col justify-between transition-colors">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 text-xs font-bold">
                      2
                    </span>
                    <h3 className="font-bold text-slate-700 dark:text-slate-200">
                      翻訳を実行
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <span className="material-icons-outlined text-lg group-hover:rotate-180 transition-transform duration-500">
                        sync
                      </span>
                      再翻訳を実行
                    </>
