...
export const ExportSettings: React.FC<ExportSettingsProps> = ({
    ...
    const updatedPresets = customPresets.filter(
      (p) =>
        !(
          ...
          p.height === presetToDelete.height
        ),
    );
    saveCustomPresets(updatedPresets);
  };
  ...
  return (
    <div
      className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-border-light dark:border-border-dark shadow-sm relative overflow-hidden space-y-6 animate-fade-in-up"
      ...
      <div className="space-y-5">
        ...
        <div className="group">
          <label
            ...
            className="block text-xs font-semibold tracking-wide text-slate-500 dark:text-slate-400 uppercase mb-2 group-focus-within:text-primary transition-colors"
          >
            ファイル名
          </label>
        ...
        <div className="group">
          ...
          <div className="relative">
            ...
            >
              ...
              {customPresets.length > 0 && (
                ...
                >
                  {customPresets.map((preset) => (
                    <option
                      key={`custom-${preset.name}-${preset.width}x${preset.height}`}
              ...
              <option
                value="custom"
                className="bg-white dark:bg-slate-800 font-semibold text-primary"
              >
                カスタムサイズを設定...
              </option>
            </select>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-border-light dark:border-slate-700/50">
            ...
            <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1">
              {customPresets.map((preset) => (
                <div
                  key={`${preset.name}-${preset.width}x${preset.height}`}
                  className="flex items-center justify-between bg-white dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 px-3 py-1.5 rounded-lg text-sm transition-colors border border-border-light/50 dark:border-slate-600/30"
                >
              ...
              <div className="pt-2">
                {!showSaveDialog ? (
                  ...
                  <div className="space-y-3 p-3 bg-white dark:bg-slate-900/50 rounded-lg border border-border-light dark:border-slate-700/50 animate-fade-in">
                    <div>
                      <label className="block text-[10px] font-medium text-slate-500 dark:text-slate-400 mb-1.5">
                        プリセット名
                      </label>
                      <input
                        type="text"
          ...
          {showAdvanced && (
            <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-border-light dark:border-slate-700/50 animate-fade-in-up">
              <div className="flex justify-between items-center mb-2">
                <label
                  htmlFor="export-dpi-range"
                  className="block text-xs font-medium text-slate-700 dark:text-slate-300"
                >
                  解像度{" "}
                  <span className="text-slate-400 dark:text-slate-500 uppercase ml-1">
                    (DPI)
                  </span>
                </label>
