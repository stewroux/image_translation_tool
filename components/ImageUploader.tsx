...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const validateAndSelectFile = (file?: File) => {
    ...
    if (file.size > MAX_SIZE) {
      ...
      setValidationError(
        `ファイルサイズが大きすぎます（${sizeMB}MB / 最大10MB）`,
      );
      return;
    }
    ...
    if (!validTypes.includes(file.type)) {
      setValidationError(
        "対応していないファイル形式です（PNG, JPG, GIF, WebPのみ可）",
      );
      return;
    }
  ...
  return (
    ...
    >
      {validationError && (
        <div
          role="alert"
          className="mb-3 px-3 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-600 dark:text-red-400 text-xs flex items-center gap-2 animate-fade-in-up"
        >
      ...
      {previewUrl ? (
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="bg-white/90 dark:bg-slate-800/90 text-slate-700 dark:text-slate-200 px-3 py-1 rounded text-sm shadow-sm group-hover:scale-105 transition-transform">
              変更する
            </span>
          </div>
        </div>
        ...
        <div
          onClick={handleClick}
          className={`flex-grow flex flex-col justify-center items-center border-2 border-dashed rounded-lg p-8 transition-colors cursor-pointer group ${
            isDragging
